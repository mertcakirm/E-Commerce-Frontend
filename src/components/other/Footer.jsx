import './css/footer.css';
import { FaFacebookF, FaInstagram, FaYoutube } from "react-icons/fa";

const Footer = () => {
    return (
        <footer className="modern-footer-wrapper">
            <div className="container footer-container">
                {/* Üst Link ve Bilgi Bölümü */}
                <div className="row g-4 pb-5 footer-top-row">
                    {/* Kolon 1: Yardım Destek */}
                    <div className="col-12 col-md-6 col-lg-4">
                        <div className="footer-col">
                            <h6 className="footer-baslik">YARDIM VE DESTEK</h6>
                            <div className="footer-links-list">
                                <a href='/sss' className='footer-icerik'>Üyelik</a>
                                <a href='/sss' className='footer-icerik'>Sipariş Takibi</a>
                                <a href='/sss' className='footer-icerik'>Ödeme Seçenekleri</a>
                                <a href='/sss' className='footer-icerik'>Kargo ve Teslimat</a>
                                <a href='/sss' className='footer-icerik'>İade ve Değişim</a>
                                <a href='/sss' className='footer-icerik'>Sık Sorulan Sorular</a>
                                <a href='/sss' className='footer-icerik'>İşlem Rehberi</a>
                            </div>
                        </div>
                    </div>

                    {/* Kolon 2: Kurumsal */}
                    <div className="col-12 col-md-6 col-lg-4">
                        <div className="footer-col">
                            <h6 className="footer-baslik">KURUMSAL</h6>
                            <div className="footer-links-list">
                                <a href='/' className='footer-icerik'>Anasayfa</a>
                                <a href='../hakkimizda' className='footer-icerik'>Hakkımızda</a>
                                <a href='/urunler/tum-urunler' className='footer-icerik'>Ürünler</a>
                                <a href='../iletisim' className='footer-icerik'>İletişim</a>
                            </div>
                        </div>
                    </div>

                    {/* Kolon 3: Sosyal Medya & Ödeme Yöntemleri */}
                    <div className="col-12 col-lg-4">
                        <div className="footer-col">
                            <h6 className="footer-baslik">BİZİ TAKİP EDİN</h6>
                            <div className='bizi-takip-edin-footer-flex'>
                                <a href='#' className='bizi-takip-edin-item' aria-label="Instagram">
                                    <FaInstagram size={18} />
                                </a>
                                <a href='#' className='bizi-takip-edin-item' aria-label="Facebook">
                                    <FaFacebookF size={17} />
                                </a>
                                <a href='#' className='bizi-takip-edin-item' aria-label="YouTube">
                                    <FaYoutube size={18} />
                                </a>
                            </div>

                            <h6 className='footer-baslik-odeme'>GÜVENLİ ÖDEME YÖNTEMLERİ</h6>
                            <div className='odeme-yontemleri-flex'>
                                <div className="payment-card-badge">
                                    <img
                                        src="data:image/png;base64,"
                                        alt="Troy / Banka Kartları"
                                    />
                                </div>
                                <div className="payment-card-badge">
                                    <img
                                        src="https://www.businessworldglobal.com/wp-content/uploads/2019/06/1559305839_MC_Logo__1.jpg"
                                        alt="MasterCard / Visa"
                                    />
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                {/* Alt Yasal / KVKK Barı */}
                <div className="kvkk-row">
                    <div className="kvkk-flex">
                        <a className='kvkk-item' href="/bilgilendirmeler">Kullanım Koşulları</a>
                        <span className="kvkk-divider">•</span>
                        <a className='kvkk-item' href="/bilgilendirmeler">Gizlilik ve Güvenlik</a>
                        <span className="kvkk-divider">•</span>
                        <a className='kvkk-item' href="/bilgilendirmeler">Tüketici Hakları</a>
                        <span className="kvkk-divider">•</span>
                        <a className='kvkk-item' href="/bilgilendirmeler">Mesafeli Satış Sözleşmesi</a>
                        <span className="kvkk-divider">•</span>
                        <a className='kvkk-item' href="/bilgilendirmeler">KVKK Aydınlatma Metni</a>
                    </div>
                    <div className="footer-copyright">
                        © {new Date().getFullYear()} Tüm Hakları Saklıdır.
                    </div>
                </div>
            </div>
        </footer>
    );
};

export default Footer;
